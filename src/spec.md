...
```js
let promise = webxdc.setUpdateListener((update) => {}, serial);
```
...
```js
let promise = webxdc.sendToChat(message);
```
...
```js
let files = await webxdc.importFiles(filter);
```
...
// async/await
try {
  let files = await webxdc.importFiles({
    mimeTypes: ["text/calendar"],
    extensions: [".ics"],
  })
  /* do sth with the files */
